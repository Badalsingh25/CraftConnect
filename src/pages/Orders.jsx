...
export default function Orders() {
  ...
  async function fetchOrders() {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/orders/mine?${params.toString()}`, {
  ...
  async function updateStatus(id, status) {
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        console.error('Order status update failed', { url, status: res.status, data });
    ...
    <div className="p-6">
        ...
        <table className="w-full text-left">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td className="p-4" colSpan={7}>No orders yet.</td></tr>
            ) : (
              orders.map((o) => (
                <tr key={o._id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-3 flex items-center gap-3">
                    {o.product?.image && (
                      <img src={getImageUrl(o.product.image)} alt={o.product?.name} className="w-10 h-10 object-cover rounded" />
                    )}
                  ...
                  <td className="p-3">
                    {o.status === 'Pending' && (
                      <div className="flex gap-2">
                        <button onClick={() => updateStatus(o._id, 'Shipped')} className="px-3 py-1 text-xs bg-indigo-600 text-white rounded">Mark Shipped</button>
                        <button onClick={() => updateStatus(o._id, 'Cancelled')} className="px-3 py-1 text-xs bg-red-600 text-white rounded">Cancel</button>
                      </div>
                    )}
                    {o.status === 'Shipped' && (
                      <button onClick={() => updateStatus(o._id, 'Delivered')} className="px-3 py-1 text-xs bg-green-600 text-white rounded">Mark Delivered</button>
                    )}
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button disabled={currentPage===1} onClick={()=>setPage(p=>Math.max(1, p-1))} className="px-3 py-2 border rounded disabled:opacity-50">Previous</button>
          <button disabled={currentPage===totalPages} onClick={()=>setPage(p=>Math.min(totalPages, p+1))} className="px-3 py-2 border rounded disabled:opacity-50">Next</button>
          <select value={pageSize} onChange={(e)=>{ setPageSize(Number(e.target.value)); setPage(1); }} className="px-2 py-2 border rounded">
